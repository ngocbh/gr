#!/usr/bin/env bash
# Three-seed ML-1M full/local/hybrid FoHSTU comparison.
# Submit with: bash scripts/submit_attention_experiments.sh hybrid

#SBATCH --job-name=fohstu-hybrid
...
#SBATCH --array=0-14
#SBATCH --requeue
#SBATCH --output=/checkpoints/ngocbh/longhstu/logs/slurm/fohstu_hybrid_%A_%a.out
#SBATCH --error=/checkpoints/ngocbh/longhstu/logs/slurm/fohstu_hybrid_%A_%a.err
...
configs=(
  ...
  "configs/ml-1m/hstu-hybrid-forgetting-w32-large-final.gin"
  "configs/ml-1m/hstu-local-forgetting-w64-large-final.gin"
  "configs/ml-1m/hstu-hybrid-forgetting-w64-large-final.gin"
)
variants=(
  "fohstu-full"
  "fohstu-local-w32"
...
master_port=$((26000 + (SLURM_JOB_ID + task_id) % 14000))
...
bash scripts/train.sh research "$config" \
  ...
  "--gin_bindings=train_fn.wandb_tags=['ml-1m','fohstu-hybrid','seed-$seed']" \
