#!/usr/bin/env bash
# Initial FoHSTU mechanism sweep on ML-1M.
# Submit with: bash scripts/submit_attention_experiments.sh fohstu

#SBATCH --job-name=fohstu-ml1m
...
#SBATCH --array=0-2
#SBATCH --requeue
#SBATCH --output=/checkpoints/ngocbh/longhstu/logs/slurm/fohstu_ml1m_%A_%a.out
#SBATCH --error=/checkpoints/ngocbh/longhstu/logs/slurm/fohstu_ml1m_%A_%a.err
...
names=(
  "ml1m-hstu-control-seed42"
  "ml1m-hstu-fixed-forgetting-seed42"
  "ml1m-fohstu-learned-seed42"
)
...
bash scripts/train.sh research "$config" \
  ...
  "--gin_bindings=train_fn.wandb_tags=['ml-1m','fohstu','mechanism','seed-42']" \
