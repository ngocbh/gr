#!/usr/bin/env bash
# Seed-42 ML-20M full/local/LIFT FoHSTU target-regression comparison.
# Submit with: bash scripts/submit_attention_experiments.sh lift-ml20

#SBATCH --job-name=lift-ml20
...
#SBATCH --output=/checkpoints/ngocbh/longhstu/logs/slurm/lift_ml20_%A_%a.out
#SBATCH --error=/checkpoints/ngocbh/longhstu/logs/slurm/lift_ml20_%A_%a.err

set -euo pipefail

: "${GR_CODE_SNAPSHOT:?submit through scripts/submit_attention_experiments.sh lift-ml20}"
...
run_name="ml20m-${variant}-seed42"
array_job_id="${SLURM_ARRAY_JOB_ID:-$SLURM_JOB_ID}"
restart_count="${SLURM_RESTART_COUNT:-0}"
# A requeue deliberately starts again from seed 42. The rN suffix isolates the
# retry's checkpoints and W&B run instead of attempting an unsafe implicit resume.
...
echo "job=$SLURM_JOB_ID task=$task_id host=$(hostname) config=$config variant=$variant"
...
bash scripts/train.sh research "$config" \
  ...
  "--gin_bindings=train_fn.wandb_tags=['ml-20m','lift-ml20','target-regression','$variant','seed-42']" \
