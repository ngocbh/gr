#!/usr/bin/env bash
# Parameter-matched Signed-LIFT old-tail feature screen on ML-1M.
# Submit with: bash scripts/submit_attention_experiments.sh signed-lift

#SBATCH --job-name=signed-lift-ml1m
...
#SBATCH --output=/checkpoints/ngocbh/longhstu/logs/slurm/signed_lift_ml1m_%A_%a.out
#SBATCH --error=/checkpoints/ngocbh/longhstu/logs/slurm/signed_lift_ml1m_%A_%a.err

set -euo pipefail

: "${GR_CODE_SNAPSHOT:?submit through scripts/submit_attention_experiments.sh signed-lift}"
...
configs=(
  "configs/ml-1m/hstu-local-forgetting-w32-large-final.gin"
  "configs/ml-1m/hstu-signed-lift-identity-w32-large-final.gin"
  "configs/ml-1m/hstu-signed-lift-tanh-w32-large-final.gin"
  "configs/ml-1m/hstu-signed-lift-abs-tanh-w32-large-final.gin"
)
names=(
  "ml1m-signed-lift-local-w32"
  "ml1m-signed-lift-identity-w32"
  "ml1m-signed-lift-tanh-w32"
  "ml1m-signed-lift-abs-tanh-w32"
)
tags=(
  "local-control"
  "identity-tail"
...
master_port=$((25000 + (SLURM_JOB_ID + task_id) % 15000))

echo "job=$SLURM_JOB_ID task=$task_id host=$(hostname) config=$config seed=$seed"
echo "screen=signed-lift-tail-features parameter_match=exact-local-lift"
...
bash scripts/train.sh research "$config" \
  ...
  "--gin_bindings=train_fn.wandb_tags=['ml-1m','signed-lift','$mode_tag','seed-$seed','parameter-matched']" \
