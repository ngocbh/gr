...
class SlurmContractTest(unittest.TestCase):
    ...
    def test_submission_is_gated_on_qualification_for_selected_datasets(self) -> None:
        ...
        self.assertIn('log_root="$repo_root/logs"', submitter)
        self.assertIn('--output="$log_root/slurm-%j.out"', submitter)
        self.assertIn('--error="$log_root/slurm-%j.out"', submitter)
        self.assertIn('--output="$log_root/slurm-%A_%a.out"', submitter)
        self.assertIn('--error="$log_root/slurm-%A_%a.out"', submitter)
