...
"""
HSTU with Attention Residuals (AttnRes), a copy of hstu.py with one change: the
fixed per-block residual ``x + f(x)`` is replaced by an input-dependent softmax
mix over all previous layer (block) outputs.

AttnRes (https://arxiv.org/abs/2603.15031, MoonshotAI / Kimi Team) computes each
layer's input as ``h_l = sum_{i<l} alpha_{i->l} * v_i`` where the weights come from
a single learned pseudo-query ``w_l`` per layer:
``alpha_{.->l} = softmax_i( w_l . RMSNorm(v_i) )``.

Base HSTU: Actions Speak Louder than Words (https://arxiv.org/abs/2402.17152, ICML'24).
"""
...
class SequentialTransductionUnitJagged(torch.nn.Module):
    ...
    def forward(  # pyre-ignore [3]
        ...
        # Delta only: the residual is reconstructed by the AttnRes mix in HSTUJagged.
        new_outputs = self._o(
            F.dropout(
                o_input,
...
class _AttnResidual(torch.nn.Module):
    """Attention Residual: softmax mix over previous layer/block representations.

    Given reps ``[v_0, ..., v_{m-1}]`` (each a jagged ``[L, D]`` tensor), returns
    ``h = sum_i softmax_i( w . RMSNorm(v_i) ) * v_i`` where ``w`` is this call site's
    learned pseudo-query (``proj.weight``). The softmax is over the depth axis and is
    per-token (each of the L positions gets its own weights). proj.weight is zero-init
    so the mix starts uniform (a DenseFormer-style average) and learns selectivity.
    """
    ...
    def __init__(self, embedding_dim: int, eps: float = 1e-6) -> None:
        ...
        torch.nn.init.zeros_(self._proj.weight)  # uniform softmax at init
...
class HSTUJagged(torch.nn.Module):
    def __init__(
        ...
        self._attn_res: torch.nn.ModuleList = torch.nn.ModuleList(
            [_AttnResidual(embedding_dim) for _ in modules]
        )
    ...
    def jagged_forward(
        ...
        if delta_x_offsets is not None or cache is not None:
            # The cross-layer residual mix can't be reconstructed from the per-layer cache.
            raise NotImplementedError(
                "HSTU-AttnRes does not support incremental/cached decoding "
                "(delta_x_offsets / cache); use the standard full forward."
            )
        ...
        with torch.autocast(
            ...
            # AttnRes residual stream: ``blocks`` holds completed block reps (incl. the
            # input token embedding x0); ``partial`` is the current block's additive
            # partial sum. Each layer's input is a softmax mix over blocks + partial.
            ...
            x = partial if partial is not None else blocks[-1]
...
class HSTUAttnRes(SequentialEncoderWithLearnedSimilarityModule):
    """
    HSTU with Attention Residuals (AttnRes / Block AttnRes), https://arxiv.org/abs/2603.15031.

    Identical to HSTU (Actions Speak Louder than Words, https://arxiv.org/abs/2402.17152)
    except the fixed per-block residual is replaced by an input-dependent softmax mix
    over previous block representations (see module docstring). ``attnres_block_size``
    sets the number of HSTU layers per AttnRes block (1 = Full AttnRes).
    ...
    def debug_str(self) -> str:
        ...
        if not self._enable_relative_attention_bias:
            debug_str += "-norab"
        debug_str += f"-attnres-bs{self._attnres_block_size}"
        return debug_str
