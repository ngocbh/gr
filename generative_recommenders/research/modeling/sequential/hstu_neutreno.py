...
"""
NeuTRENO variant of HSTU. This file is a copy of hstu.py with one small change:
each attention block adds the NeuTRENO term ``neutreno_lambda * (v_0 - v_l)`` to its
attention output, where ``v_0`` is the value matrix from the FIRST block and ``v_l``
the value at the current block (the v-split of uvqk). This re-injects the initial,
diverse representation to counteract oversmoothing (token reps collapsing to a common
direction with depth). ``neutreno_lambda = 0`` recovers vanilla HSTU.

Ref: "Mitigating Oversmoothing in Transformers via Regularized Nonlocal Functionals"
(NeuTRENO), https://openreview.net/pdf?id=3fd776zKmo.
...
class SequentialTransductionUnitJagged(torch.nn.Module):
    def __init__(
        ...
        super().__init__()
        # NeuTRENO strength: when > 0, forward() adds lambda * (v_0 - v) to the
        # attention output. 0 == vanilla HSTU block.
        self._neutreno_lambda: float = neutreno_lambda
        # Where to inject the term relative to the attn-output LayerNorm:
        #   False (default) -> before norm: norm(attn_output + lambda*(v_0 - v))
        #   True            -> after  norm: norm(attn_output) + lambda*(v_0 - v)
        self._neutreno_after_norm: bool = neutreno_after_norm
    ...
    def forward(  # pyre-ignore [3]
        ...
        # === NeuTRENO term: lambda * (v_0 - v) ===
        # v_0 is the first block's value (None for the first block, where the term
        # is zero). Applied only on the standard (non-incremental) path. The term is
        # injected either BEFORE the attn-output norm (default) or AFTER it
        # (neutreno_after_norm), see _neutreno_after_norm.
        neutreno_term = None
        if self._neutreno_lambda > 0.0 and v_0 is not None and delta_x_offsets is None:
            neutreno_term = self._neutreno_lambda * (v_0 - v)

        if neutreno_term is not None and not self._neutreno_after_norm:
            attn_output = attn_output + neutreno_term

        a = self._norm_attn_output(attn_output)

        if neutreno_term is not None and self._neutreno_after_norm:
            a = a + neutreno_term
...
class HSTUJagged(torch.nn.Module):
    ...
    def jagged_forward(
        ...
        # NeuTRENO: capture the first block's value (v_0) and pass it to every
        # subsequent block so they can add lambda * (v_0 - v_l) to attn output.
        v_0: Optional[torch.Tensor] = None
        with torch.autocast(
            "cuda",
            ...
            for i, layer in enumerate(self._attention_layers):
                ...
                if i == 0:
                    # cache_states_i = (v, padded_q, padded_k, new_outputs); v is [0].
                    v_0 = cache_states_i[0]
...
class HSTUNeuTRENO(SequentialEncoderWithLearnedSimilarityModule):
    """
    HSTU with the NeuTRENO anti-oversmoothing term (see module docstring).
    ...
    def debug_str(self) -> str:
        ...
        if not self._enable_relative_attention_bias:
            debug_str += "-norab"
        if self._neutreno_lambda > 0.0:
            debug_str += f"-neutreno{self._neutreno_lambda}"
            if self._neutreno_after_norm:
                debug_str += "-postnorm"
        return debug_str
